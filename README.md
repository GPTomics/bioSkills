...
| **crispr-screens** | 15 | MAGeCK, JACKS, CRISPResso2, BAGEL2 | Pooled screen analysis, sgRNA efficacy modeling, hit calling, base/prime editing |
...
| **reproducibility** | 3 | BioFlow-Insight, nf-test, pytest-workflow, ro-crate-py, runcrate, cwltool | Reproducibility by design: availability vs reproducibility vs reuse and the three levels, modular design with explicit I/O contracts and tool identity (bio.tools/EDAM, CoPaLink), shareable synthetic test data and regression/snapshot testing that catches silent tool-version drift, and fine-grained (why-)provenance vs overload with Workflow Run RO-Crate/CWLProv/BioCompute Objects and WorkflowHub/Dockstore/GA4GH publishing |
...
"Restructure my monolithic pipeline into reusable modules and reconstruct its structure from the code"
"Generate tiny synthetic test data and expected outputs so my workflow can be regression-tested in CI"
"Package a completed run as a Workflow Run RO-Crate and publish it to WorkflowHub with a DOI"
